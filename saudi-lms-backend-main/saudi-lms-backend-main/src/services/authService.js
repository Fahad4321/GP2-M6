...
const registerService = async ({
  ...
  if (send[0].statusCode === 202) {
    let isRole = await rolePermissionService.roleFindByProperty("name", role);
    if (!isRole) {
...
const setPasswordService = async ({ token, password }) => {
  const decoded = jwt.verify(token, process.env.TOKEN_SECRET);
  const user = await findOneByQuery(
    { email: decoded?.email, confirmationToken: token },
    UserModel
  );
  ...
  user.verified = true;
  user.save();
  return userService.passwordUpdateService({
    email: decoded.email,
    hash: hashPassword,
  });
};
