...
const createContent = async (contents) => {
  const errors = (
    await Promise.all(
      contents.map(async (content) => {
        ...
        const moduleErrors = await Promise.all(
          modules.map(async (module) => {
            const isTitle = await CourseContent.findOne({
              moduleId: module?._id,
              videoTitle: content?.videoTitle?.toLowerCase()?.trim(),
            });
            const isUrl = await CourseContent.findOne({
              moduleId: module?._id,
              videoUrl: content?.videoUrl?.toLowerCase()?.trim(),
            });
            const isSerial = await CourseContent.findOne({
            ...
            if (isTitle) {
              ...
              isUrl?.videoUrl.toLowerCase() === content?.videoUrl.toLowerCase()
            ) {
  ...
  if (!isVideoTitleNoUnique) {
    throw error(
      "You provide same video title, please provide different video title",
      400
    );
  }
  if (!isVideoUrlNoUnique) {
    throw error(
      "You provide same video url, please provide different video url",
      400
    );
  }
  if (!isSerialNoNoUnique) {
    throw error(
      "You provide same serial no, please provide different serial no",
      400
    );
  }
